...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(books.id) FROM characters, books LEFT OUTER JOIN character_books ON characters.id = character_books.character_id WHERE characters.id = character_books.character_id;"
end
