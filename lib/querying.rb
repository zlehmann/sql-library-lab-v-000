...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(books.id) FROM characters, books LEFT OUTER JOIN series ON characters.series_id = series.id WHERE books.series_id = series.id GROUP BY characters.name HAVING COUNT(books.id);"
end
